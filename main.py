...
with open('score.txt', 'a') as f1:  # Открытие для добавления, чтобы создасть файл если его нет
...
class Enemy(pygame.sprite.Sprite):
    ...
    def update(self):
        ...
        if self.health:   # Если враг жив
            ...
            SHOTS = popped_shots
        if self.health == 0 and self.k == 0:  # Если враг только что уничтожен
            ...
            self.rect.x += w // 4
            self.rect.y += h // 4
            s2 = pygame.mixer.Sound('data/Sound/hit1.wav')  # Звук уничтожения
            s2.play()
...
class Meteorite(Enemy):
    ...
    def __init__(self):
        ...
        self.image = pygame.transform.scale(load_image("Picture/Meteorite_1.png"), (80, 80))  # Изображение
...
class MeteoriteBoss(Enemy):
    ...
    def __init__(self):
        ...
        self.image = pygame.transform.scale(load_image("Picture/Meteorite_2.png"), (150, 150))  # Изображение
        self.rect = self.image.get_rect()  # Занимаемый прямоугольник
        self.mask = pygame.mask.from_surface(self.image)  # Маска для коллизий
...
class AsteroidBoss(Enemy):
    ...
    def __init__(self):
        ...
        self.score = self.health  # Очки
        self.speed = 15  # Скорость
        self.image = pygame.transform.scale(load_image("Picture/Asteroid_2.png"), (150, 150))  # Изображение
        self.rect = self.image.get_rect()  # Занимаемый прямоугольник
        self.mask = pygame.mask.from_surface(self.image)  # Маска для коллизий
...
class Satellite(Enemy):
    ...
    def __init__(self):
        ...
        self.score = -60  # Очки
        self.speed = 15  # Скорость
        self.image = pygame.transform.scale(load_image("Picture/Satellite.png"), (80, 80))  # Изображение
        self.rect = self.image.get_rect()  # Занимаемый прямоугольник
        self.mask = pygame.mask.from_surface(self.image)  # Маска для коллизий
...
class Shot(pygame.sprite.Sprite):
    ...
    def __init__(self, x, y):
        ...
        self.cut_sheet(load_image("Picture/shot.png"))  # Нарезка спрайт-листа
        self.cur_frame = 0  # Текущий кадр
        self.k = 0  # Счётчик для анимации
